...
3. Verify the Conda GPU PyTorch installation.
4. Verify that PyTorch detects the A100.
5. Configure the full train and test paths.
6. Run the regression tests.
7. Prepare and validate the dataset.
8. Start the training pipeline.
...
Do not execute it yet. Complete Sections 2 through 7 first. Do not use the
...
This installs Python 3.11, GPU PyTorch, Transformers, NumPy, CPU FAISS,
PyYAML, tqdm, and the test runner from `conda-forge`. CPU FAISS is intentional:
model encoding runs on the A100, while exact search over the expected Ruby
corpus is small enough to run on CPU.
...
PyTorch stopped publishing new official Conda packages starting with PyTorch
2.6. The `pytorch-gpu` package in this environment is therefore the
`conda-forge` build, not the deprecated official `pytorch` channel.

### 2.1 Fallback when `pytorch-gpu` cannot use the A100

If the cluster cannot solve `pytorch-gpu`, create a Conda environment first:

```bash
conda create -n ar2 python=3.11 pip -y
conda activate ar2
python -m pip install -r requirements-code-search.txt
```

Then install CUDA-enabled PyTorch inside that Conda environment using the
command generated by the official selector:

```text
https://docs.pytorch.org/get-started/locally/
```

Choose Linux, Pip, Python, and a CUDA build compatible with the server's NVIDIA
driver. This fallback still uses Conda to manage the environment; only PyTorch
and the fallback project dependencies are installed as wheels.
...
```bash
conda install -n ar2 -c conda-forge faiss-cpu
```
